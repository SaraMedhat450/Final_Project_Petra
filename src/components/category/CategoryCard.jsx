        ...
        >
                ...
                {imageUrl ? (
                    <img 
                        ...
                        onError={(e) => { 
                            ...
                            e.target.parentElement.innerHTML = '<div class="w-full h-full flex flex-col items-center justify-center bg-[#F2F9F9] text-[#176B87]"><svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1" stroke-linecap="round" stroke-linejoin="round"><rect width="7" height="7" x="3" y="3" rx="1"/><rect width="7" height="7" x="14" y="3" rx="1"/><rect width="7" height="7" x="14" y="14" rx="1"/><rect width="7" height="7" x="3" y="14" rx="1"/></svg></div>'; 
                        }}
                    />
                ) : (
                    <div className="w-full h-full flex flex-col items-center justify-center bg-[#F2F9F9] text-[#176B87]">
            ...
            <div className="flex-1 flex flex-col p-8 items-center text-center">
                <Link 
                    ...
                    className="text-2xl font-black tracking-tight mb-2 text-[#04364A] hover:text-[#176B87] transition-all line-clamp-1"
                >
                    {category.name}
                </Link>
                ...
                <div className="flex items-center gap-3 mb-6 justify-center">
                    {category.rating > 0 ? (
                        <div className="flex items-center gap-2 bg-yellow-50 px-3 py-1 rounded-full border border-yellow-100 shadow-sm">
                            ...
                            <span className="text-[10px] font-black text-[#F59E0B] tracking-tight">{category.rating} RATING</span>
                        </div>
                ...
                <div className="w-full mb-auto pb-4">
                    <div className="flex items-center justify-center gap-3 mb-5">
                        <div className="w-6 h-[1px] bg-[#176B87] opacity-30"></div>
                        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-[#04364A] whitespace-nowrap">Subcategories</span>
                        <div className="w-6 h-[1px] bg-[#176B87] opacity-30"></div>
                    </div>
                    ...
                    <div className="flex flex-wrap justify-center gap-2.5 px-2">
                        {subcats.length > 0 ? (
                            <>
                                {subcats.slice(0, 4).map(sub => (
                                    <Link 
                                        ...
                                        className="text-[10px] px-4 py-2 bg-white text-[#04364A] rounded-2xl font-black tracking-tight hover:bg-[#176B87] hover:text-white transition-all duration-300 border border-gray-100 hover:border-transparent hover:shadow-md"
                                    >
                                        {sub.name}
                                    </Link>
                                ))}
                                {subcats.length > 4 && (
                                    <Link 
                                        to={`/subcategories?categoryId=${category.id}&categoryName=${encodeURIComponent(category.name)}`}
                                        className="text-[10px] px-4 py-2 bg-white/50 text-[#04364A]/60 rounded-2xl font-black border border-gray-200/30 hover:bg-white hover:text-[#04364A] transition-all shadow-sm"
                                    >
                            ...
                            <p className="text-[11px] font-bold text-[#04364A]/60 italic leading-relaxed px-6">
